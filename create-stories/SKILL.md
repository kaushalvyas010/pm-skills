...
- **Acceptance Criteria** — 3-6 specific, testable conditions. Written in Given/When/Then
  format: "Given [context or precondition], When [the user does X], Then [observable
  outcome]." No implementation language. A QA engineer should be able to write tests
  from these without asking questions.
...
(S/M/L) and any stories that should be split before committing to a sprint. When
splitting is needed, use SPIDR to find the right cut: Split by Spike (learn first),
Path (one user flow at a time), Interface (channel or device), Data (subset of inputs
or outputs), or Rules (one business rule at a time). Don't split arbitrarily by effort —
split by the seam that preserves independent, deliverable value.
